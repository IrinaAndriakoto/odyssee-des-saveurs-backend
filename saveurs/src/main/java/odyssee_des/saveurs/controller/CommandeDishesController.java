...
public class CommandeDishesController {
    ...
    @GetMapping("/getAll")
    public ResponseEntity<List<CommandeDishDTO>> getAllCommandeDishes() { return ResponseEntity.ok(service.getAllCommandeDishes()); }

    @GetMapping("/{id}")
    public ResponseEntity<CommandeDishDTO> getCommandeDishesById(@PathVariable Long id) {
        return service.getCommandeDishesById(id).map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @GetMapping("/{id}/dishes")
    public ResponseEntity<List<CommandeDishDTO>> getDishesByCommande(@PathVariable("id") Long id) {
        List<CommandeDishDTO> list = service.getAllDishesByCommandeId(id);
        return ResponseEntity.ok(list);
    }

    @PostMapping("/add")
    public ResponseEntity<CommandeDishDTO> addCommandeDishes(@RequestBody CommandeDishDTO cd) {
        return ResponseEntity.status(201).body(service.addCommandeDishes(cd));
    }

    @PutMapping("/update/{id}")
    public ResponseEntity<CommandeDishDTO> updateCommandeDishes(@PathVariable Long id, @RequestBody CommandeDishDTO cd) {
        cd.setId(id);
        CommandeDishDTO upt = service.updateCommandeDishes(cd);
